...
state_option = st.selectbox('Select State', [row[0] for row in state_list])
...
if (state_option is not None and len(state_option) > 1):
    ...
    city_option = st.selectbox('Select City', [row[0] for row in city_list])
...
if (city_option is not None and len(city_option) > 1):
    ...
    station_option = st.selectbox('Select Station',[row[0] for row in station_list])
...
if (station_option is not None and len(station_option) > 1):
    ...
    date_option = st.selectbox('Select Date', [row[0] for row in date_list])
...
if (date_option is not None):
    ...
    from 
        ...
        join 
        dev_db.consumption_sch.location_dim l on l.location_ID  = f.location_fk and 
